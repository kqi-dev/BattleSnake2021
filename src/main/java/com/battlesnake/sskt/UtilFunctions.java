...
public class UtilFunctions {
  ...
  private static void floodfillHelper(CoordinatePair head, int dist, int[][] ret) {
    ...
    floodfillHelper(new CoordinatePair(head.x + 1, head.y), dist + 1, ret);floodfillHelper(new CoordinatePair(head.x - 1, head.y), dist + 1, ret);floodfillHelper(new CoordinatePair(head.x, head.y + 1), dist + 1, ret);floodfillHelper(new CoordinatePair(head.x, head.y - 1), dist + 1, ret);
  }
}